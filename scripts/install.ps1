...
function Get-Repo {
    if (Test-Path "docker-compose.yml") {
        ...
        if ($content -match "secopsmonitor") {
            Write-Ok "Already in SecOpsMonitor directory"
            return
        }
    }

    Write-Step 2 5 "Cloning SecOpsMonitor..."
    if (Test-Path "SecOpsMonitor") {
...
function Install-Native($prereqs) {
    ...
    Write-Step 3 5 "Installing monitor-api..."
    Push-Location monitor-api
    python -m pip install -e "." --quiet
    Pop-Location
    Write-Ok "Backend dependencies installed"

    Write-Step 4 5 "Installing ui-monitor..."
    Push-Location ui-monitor
    npm install --silent
    Pop-Location
    Write-Ok "Frontend dependencies installed"

    Write-Step 5 5 "Starting SecOpsMonitor..."

    # Start monitor-api
    $monitor-apiJob = Start-Job -ScriptBlock {
        Set-Location $using:PWD\monitor-api
        python -m uvicorn app.main:app --host 0.0.0.0 --port 8000
    }
    Write-Ok "Backend starting (Job ID: $($monitor-apiJob.Id))"

    # Start ui-monitor
    $ui-monitorJob = Start-Job -ScriptBlock {
        Set-Location $using:PWD\ui-monitor
        npm run dev -- --port 5174
    }
    Write-Ok "Frontend starting (Job ID: $($ui-monitorJob.Id))"

    Start-Sleep -Seconds 3

    # Save job IDs
    "$($monitor-apiJob.Id)`n$($ui-monitorJob.Id)" | Out-File .secopsmonitor.jobs
}
...
if ($prereqs.Docker) {
    ...
    Install-Docker
    $ui-monitorUrl = "http://localhost:3000"
    $apiUrl = "http://localhost:8000/docs"
    $stopCmd = "docker compose down"
} else {
    Write-Ok "Using native installation"
    Install-Native $prereqs
    $ui-monitorUrl = "http://localhost:5174"
    $apiUrl = "http://localhost:8000/docs"
    $stopCmd = "Get-Content .secopsmonitor.jobs | ForEach-Object { Stop-Job -Id `$_ }"
}

Write-Host ""
Write-Host "════════════════════════════════════════════════" -ForegroundColor Green
Write-Host "  SecOpsMonitor is running!" -ForegroundColor Green
Write-Host "════════════════════════════════════════════════" -ForegroundColor Green
Write-Host ""
Write-Host "  Frontend:  $ui-monitorUrl" -ForegroundColor White
