# SecOpsMonitor Installation Guide

Complete step-by-step instructions for installing and running SecOpsMonitor on your system.
...
4. [Running SecOpsMonitor](#running-secopsmonitor)
...
```bash
# Navigate to ui-monitor directory
cd ui-monitor
...
Create a `.env.local` file in the `ui-monitor` directory for custom settings:
...
# Output will be in ui-monitor/dist/
...
```bash
# Build Docker image
docker build -t secopsmonitor:latest .

# Run container
docker run -d \
  --name secopsmonitor \
  -p 5174:5174 \
  -v secopsmonitor-data:/app/data \
  secopsmonitor:latest
...
kubectl port-forward svc/secopsmonitor 5174:5174
```
...
# Docker
docker build -t secopsmonitor:latest .
docker run -p 5174:5174 secopsmonitor:latest
docker-compose up -d
