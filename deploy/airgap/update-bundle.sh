#!/usr/bin/env bash
# =============================================================================
# SecOpsMonitor Air-Gap Update Script
# =============================================================================
# Safely applies an updated air-gap bundle to a running SecOpsMonitor deployment.
# Backs up the database, loads new images, runs migrations, and restarts.
#
# Usage:  ./update-bundle.sh /path/to/secopsmonitor-airgap-bundle.tar.gz
# =============================================================================
...
# ---------------------------------------------------------------------------
if [[ $# -lt 1 ]]; then
    echo "Usage: $0 /path/to/secopsmonitor-airgap-bundle.tar.gz"
    echo ""
    echo "Applies an updated air-gap bundle to a running SecOpsMonitor deployment."
...
echo ""
echo -e "${BOLD}========================================${NC}"
echo -e "${BOLD}  SecOpsMonitor Air-Gap Update${NC}"
...
[[ -f "${ENV_FILE}" ]]          || error "No .env file found. Is SecOpsMonitor installed here?"
[[ -f "${COMPOSE_FILE}" ]]      || error "No compose file found. Is SecOpsMonitor installed here?"

# Check if services are running
RUNNING=$(${COMPOSE_CMD} -f "${COMPOSE_FILE}" ps -q 2>/dev/null | wc -l | tr -d ' ')
if [[ "${RUNNING}" == "0" ]]; then
    warn "No SecOpsMonitor services are currently running."
fi
...
[[ -f "${STAGING_DIR}/secopsmonitor-images.tar" ]] || error "No images tarball found in bundle."
...
BACKUP_FILE="${BACKUP_DIR}/secopsmonitor-db-${TIMESTAMP}.sql.gz"
...
if [[ "${RUNNING}" != "0" ]]; then
    ...
    ${COMPOSE_CMD} -f "${COMPOSE_FILE}" exec -T postgres \
        pg_dump -U "${POSTGRES_USER:-secopsmonitor}" "${POSTGRES_DB:-secopsmonitor}" \
        | gzip > "${BACKUP_FILE}"
...
docker load -i "${STAGING_DIR}/secopsmonitor-images.tar"
...
${COMPOSE_CMD} -f "${COMPOSE_FILE}" exec -T monitor-api \
...
check_service "PostgreSQL" "${COMPOSE_CMD} -f ${COMPOSE_FILE} exec -T postgres pg_isready -U secopsmonitor" || HEALTH_FAILURES=$((HEALTH_FAILURES + 1))
...
if [[ ${HEALTH_FAILURES} -gt 0 ]]; then
    ...
    echo "    gunzip -c ${BACKUP_FILE} | docker exec -i secopsmonitor-postgres psql -U secopsmonitor secopsmonitor"
