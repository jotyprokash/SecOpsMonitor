#!/usr/bin/env bash
# =============================================================================
# SecOpsMonitor Air-Gap Loader and Runner
# =============================================================================
...
IMAGES_TAR="${SCRIPT_DIR}/secopsmonitor-images.tar"
CHECKSUMS_FILE="${SCRIPT_DIR}/checksums.sha256"
VERSION_FILE="${SCRIPT_DIR}/VERSION"

echo ""
echo -e "${BOLD}========================================${NC}"
echo -e "${BOLD}  SecOpsMonitor Air-Gap Deployment${NC}"
...
# List loaded images
info "Available SecOpsMonitor images:"
docker images --format "  {{.Repository}}:{{.Tag}} ({{.Size}})" | grep -E "secopsmonitor|postgres|redis" || true
...
if [[ -f "${ENV_FILE}" ]]; then
    ...
    if [[ "$(uname)" == "Darwin" ]]; then
        ...
        sed -i '' "s|SECOPS_SECRET_KEY=GENERATE_ME|SECOPS_SECRET_KEY=${JWT_SECRET}|" "${ENV_FILE}"
    ...
    if [[ "$(uname)" == "Darwin" ]]; then
        sed -i '' "s|SECOPS_DATABASE_URL=.*|SECOPS_DATABASE_URL=postgresql+asyncpg://secopsmonitor:${DB_PASSWORD}@postgres:5432/secopsmonitor|" "${ENV_FILE}"
    else
        sed -i "s|SECOPS_DATABASE_URL=.*|SECOPS_DATABASE_URL=postgresql+asyncpg://secopsmonitor:${DB_PASSWORD}@postgres:5432/secopsmonitor|" "${ENV_FILE}"
    fi
...
# Start services
# ---------------------------------------------------------------------------
step "Starting SecOpsMonitor services"
