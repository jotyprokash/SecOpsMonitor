#!/usr/bin/env bash
# =============================================================================
# SecOpsMonitor Air-Gap Bundle Builder
# =============================================================================
...
# Output: secopsmonitor-airgap-bundle.tar.gz
# =============================================================================
...
BUNDLE_NAME="secopsmonitor-airgap-bundle"
...
echo ""
echo -e "${BOLD}========================================${NC}"
echo -e "${BOLD}  SecOpsMonitor Air-Gap Bundle Builder${NC}"
...
# Build Docker images
# ---------------------------------------------------------------------------
step "Building ui-monitor Docker image"

docker build \
    -t secopsmonitor-ui-monitor:latest \
    -t "secopsmonitor-ui-monitor:${VERSION}" \
    -f "${PROJECT_ROOT}/Dockerfile.ui-monitor" \
    "${PROJECT_ROOT}"

success "Frontend image built."

step "Building monitor-api Docker image"

docker build \
    -t secopsmonitor-monitor-api:latest \
    -t "secopsmonitor-monitor-api:${VERSION}" \
    -f "${PROJECT_ROOT}/Dockerfile.monitor-api" \
    "${PROJECT_ROOT}"
...
IMAGES_TAR="${BUILD_DIR}/secopsmonitor-images.tar"

docker save \
    secopsmonitor-ui-monitor:latest \
    "secopsmonitor-ui-monitor:${VERSION}" \
    secopsmonitor-monitor-api:latest \
    "secopsmonitor-monitor-api:${VERSION}" \
    postgres:16-alpine \
    redis:7-alpine \
    -o "${IMAGES_TAR}"
...
if [[ -f "${PROJECT_ROOT}/monitor-api/app/seed.py" ]]; then
    cp "${PROJECT_ROOT}/monitor-api/app/seed.py" "${BUILD_DIR}/seed.py"
    success "Seed script included."
else
    warn "No seed script found at monitor-api/app/seed.py — skipping."
fi
...
cd "${BUILD_DIR}"
calc_sha256 secopsmonitor-images.tar          > checksums.sha256
