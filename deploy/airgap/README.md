# SecOpsMonitor Air-Gap Deployment Guide

SecOpsMonitor is an OT/ICS security platform designed to operate in fully air-gapped
...
- Access to the SecOpsMonitor source repository
...
|----------------|----------------------------------------|--------------|
| ui-monitor       | Nginx serving the React UI             | 3000         |
| monitor-api        | FastAPI application server             | 8000         |
...
access. By default, only the ui-monitor and monitor-api ports are bound to
...
```bash
cd /path/to/SecOpsMonitor
./deploy/airgap/build-bundle.sh
```

This script will:

1. Build the ui-monitor and monitor-api Docker images.
...
The output is a single file: `secopsmonitor-airgap-bundle.tar.gz`

---

## Transferring the Bundle

1. Copy `secopsmonitor-airgap-bundle.tar.gz` to a USB drive or other approved
...
3. Copy the bundle to the target host:
   ```bash
   cp /media/usb/secopsmonitor-airgap-bundle.tar.gz /opt/secopsmonitor/
   ```
...
4. Verify the file was not corrupted during transfer (the checksum is printed
   ...
   ```bash
   sha256sum secopsmonitor-airgap-bundle.tar.gz
   ```
...
```bash
cd /opt/secopsmonitor
tar xzf secopsmonitor-airgap-bundle.tar.gz
cd secopsmonitor-airgap
./load-and-run.sh
```
...
After completion, access SecOpsMonitor at `http://localhost:3000`.
...
```bash
cd /opt/secopsmonitor/secopsmonitor-airgap
./update-bundle.sh /path/to/new/secopsmonitor-airgap-bundle.tar.gz
```
...
SecOpsMonitor is designed to operate within the Purdue Model for industrial
network segmentation. Recommended placement:

### Level 3 — Site Operations (DMZ)

This is the recommended deployment level for SecOpsMonitor. At Level 3, the
platform can:
...
|------------------------------|--------------|-----------------------------------|
| Level 2 to SecOpsMonitor (L3)    | Allow        | Scan data ingestion (read-only)   |
| SecOpsMonitor (L3) to Level 2    | Deny         | No write access to control layer  |
| Level 4 to SecOpsMonitor (L3)    | Allow        | Dashboard access for IT/OT staff  |
| SecOpsMonitor (L3) to Internet   | Deny         | Air-gapped, no external access    |
| SecOpsMonitor (L3) to Level 0-1  | Deny         | No access to field devices        |

### Firewall Configuration

If the SecOpsMonitor host is on a network segment with other systems, restrict
inbound access:
...
Verify PostgreSQL is healthy: `docker compose -f docker-compose.airgap.yml exec postgres pg_isready -U secopsmonitor`
